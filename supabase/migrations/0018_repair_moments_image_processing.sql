-- =============================================================================
-- Sprint 9.2A repair (step 2) — finish partially applied 0016
-- =============================================================================
-- Prerequisite: 0017_add_moments_processing_enum.sql committed successfully.
-- Does NOT add enum values (processing must already exist).
-- Does NOT drop tables, enums, buckets, data, or owner-only RLS.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- moment_media columns (0016 step 2)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Check constraints (0016 step 3) — skip if already present
-- ---------------------------------------------------------------------------
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'moment_media_original_path_length'
  ) then
    alter table public.moment_media
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'moment_media_processing_error_code_length'
  ) then
    alter table public.moment_media
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'moment_media_processed_size_non_negative'
  ) then
    alter table public.moment_media
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'moment_media_thumbnail_size_non_negative'
  ) then
    alter table public.moment_media
...
-- ---------------------------------------------------------------------------
-- Upload slot RPC (original + display paths)
-- ---------------------------------------------------------------------------
...
revoke all on function public.moments_parent_media_bytes(uuid) from public;
...
-- Ensure bucket stays private (no-op if already correct)
